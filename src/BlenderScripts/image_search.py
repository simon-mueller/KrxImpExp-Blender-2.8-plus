...
class TEXDIRS_UL_list_slot(bpy.types.UIList):
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        layout.label(text=item.name)

# A new panel in the user preferences
class TEXDIRS_PT_TextureDirectoriesPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row0 = layout.row()
        row0.template_list('TEXDIRS_UL_list_slot', '', sce, 'texture_directories', sce, 'selected_texture_directory', rows = 5, maxrows = 7)
...
def register():
    bpy.utils.register_class(TEXDIRS_PT_TextureDirectoriesPanel)
    bpy.utils.register_class(TEXDIRS_UL_list_slot)
...
def unregister():
    bpy.utils.unregister_class(TEXDIRS_PT_TextureDirectoriesPanel)
    bpy.utils.unregister_class(TEXDIRS_UL_list_slot)
